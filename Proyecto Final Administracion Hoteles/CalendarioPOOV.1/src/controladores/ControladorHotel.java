...
public class ControladorHotel {
    ...
    public static String[] listaReservaTabla() {
        ...
        for (int i = 0; i < mapaReservas.size(); i++) {
            if (!Boolean.parseBoolean(mapaReservas.get(i).get("asignado").toString())) {
    ...
    public static boolean estaRegistradaEmpresa(Documento documento) {
        if (!empresas.isEmpty()) {
            for (int i = 0; i < empresas.size(); i++) {
                if (empresas.get(i).getDocumento().getNumeroDocumento().equals(
    ...
    public static void agregarEmpleado(Empresa empresa, Persona persona) {
        if (!empresas.isEmpty()) {
            for (int i = 0; i < empresas.size(); i++) {
                if (empresas.get(i).getDocumento().getNumeroDocumento().equals(
                        empresa.getDocumento().getNumeroDocumento())) {
    ...
    public static boolean estaRegistradaAgencia(Documento documento) {
        if (!agencias.isEmpty()) {
            for (int i = 0; i < agencias.size(); i++) {
                if (agencias.get(i).getDocumento().getNumeroDocumento().equals(
                        documento.getNumeroDocumento())) {
    ...
    public static void agregarPasajero(Agencia agencia, Persona persona) {
        if (!agencias.isEmpty()) {
            for (int i = 0; i < agencias.size(); i++) {
                if (agencias.get(i).getDocumento().getNumeroDocumento().equals(
                        agencia.getDocumento().getNumeroDocumento())) {
...
//eliminar reserva preliminar de la tabla visual.
    ...
    public static boolean reservaRepetida(String habitacion, String fecha, int dias, String idAnterior) {
        ...
        if (!reservas.isEmpty()) {
            for (int i = 0; i < dias; i++) {
                ...
                for (int j = 0; j < reservas.size(); j++) {
                    ...
                    if (id.equals(idCorto) && !reservas.get(j).getIdReserva().equals(idAnterior)) {
    ...
    public static void editarReservaTabla(String idReservaAnterior, Habitacion habitacion, Fecha fecha,
        ...
        if (!esNulos(habitacion) && !esNulos(fecha) && dias >= 1) {
            if (!mapaReservas.isEmpty()) {
                ...
                if (reservaRepetida(habitacion.toString(), fecha.toFechaCompleta(), dias, idReservaAnterior)) {
...
//                }
    ...
    public static Reserva buscarReservaPreliminar1(String id) {
        if (esNulos(id)) {
            JOptionPane.showMessageDialog(null, "Error al ingresar datos", "Detalle de Reserva", 2);
        } else {
            if (reservas.isEmpty()) {
    ...
    public static void asignarPagador(String idReserva, Empresa empresa, int personaIndex) {

        if (esNulos(idReserva)) {
            JOptionPane.showMessageDialog(null, "Error al ingresar datos", "Detalle de Reserva", 2);
        } else {
            if (reservas.isEmpty()) {
                JOptionPane.showMessageDialog(null, "No existen Reservas para buscar", "Detalle de Reserva", 2);
                ...
                for (int i = 0; i < reservas.size(); i++) {
                    if (reservas.get(i).getIdReserva().equals(idReserva)) {
                        reservas.get(i).setCliente(empresa.getContacto().get(personaIndex));
                        reservas.get(i).setEmpresa(empresa);
                        reservas.get(i).setPendiente(true);
                    }
                }
                for (int i = 0; i < mapaReservas.size(); i++) {
                    if (mapaReservas.get(i).get("id").toString().equals(idReserva)) {
                        mapaReservas.get(i).remove("asignado");
                        mapaReservas.get(i).put("asignado", true);
                        break;
                    }
    ...
    public static void asignarPagador(String idReserva, Agencia agencia, int personaIndex) {
        ...
        if (esNulos(idReserva)) {
            ...
            if (reservas.isEmpty()) {
                ...
                for (int i = 0; i < reservas.size(); i++) {
                    if (reservas.get(i).getIdReserva().equals(idReserva)) {
                        reservas.get(i).setCliente(agencia.getContacto().get(personaIndex));
                        reservas.get(i).setAgencia(agencia);
                        reservas.get(i).setPendiente(true);
    ...
    public static void asignarPagador(String idReserva, int personaIndex) {
        ...
        if (esNulos(idReserva)) {
            ...
            if (reservas.isEmpty()) {
                ...
                for (int i = 0; i < reservas.size(); i++) {
                    if (reservas.get(i).getIdReserva().equals(idReserva)) {
                        reservas.get(i).setCliente(personas.get(personaIndex));
                        reservas.get(i).setPendiente(true);
                    }
                }
                for (int i = 0; i < mapaReservas.size(); i++) {
